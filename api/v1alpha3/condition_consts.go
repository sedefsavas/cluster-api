...
const (
	...
	// MachineNodeProvisioningReason (Severity=Info) documents machine in the process of provisioning a node.
	// NB. provisioning --> NodeRef == ""
	MachineNodeProvisioningReason = "NodeProvisioning"

	// MachineNodeDeletedReason (Severity=Error) documents machine already provisioned where a node does not exists anymore.
	// NB. provisioned --> NodeRef != ""
	MachineNodeDeletedReason = "NodeDeleted"

	// MachineNodeConditionsFailedReason (Severity=Warning) documents a node is not in a healthy state due to the failed state of at least 1 Kubelet condition.
	MachineNodeConditionsFailedReason = "NodeConditionsFailed"
)
