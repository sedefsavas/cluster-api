...
func (r *MachineReconciler) reconcileNode(ctx context.Context, cluster *clusterv1.Cluster, machine *clusterv1.Machine) error {
	...
	// Even if Status.NodeRef exists, continue to do the following checks to make sure
	...
	if err != nil {
		if err == ErrNodeNotFound {
			...
			if machine.Status.NodeRef != nil {
				conditions.MarkFalse(machine, clusterv1.MachineNodeHealthyCondition, clusterv1.MachineNodeDeletedReason, clusterv1.ConditionSeverityError, "")
			}
	...
	if machine.Status.NodeRef == nil {
		...
		conditions.MarkFalse(machine, clusterv1.MachineNodeHealthyCondition, clusterv1.MachineNodeProvisioningReason, clusterv1.ConditionSeverityWarning, "")
		return nil
	}

	// Do the remaining node health checks, then set the node health to true if all checks pass.
	if checkNodeConditions(node) == false {
		conditions.MarkFalse(machine, clusterv1.MachineNodeHealthyCondition, clusterv1.MachineNodeConditionsFailedReason, clusterv1.ConditionSeverityWarning, "")
		return nil
	}
