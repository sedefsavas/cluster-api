...
func TestKubeadmControlPlaneReconciler_initializeControlPlane(t *testing.T) {
	...
	g.Expect(machineList.Items[0].Spec.InfrastructureRef.Kind).To(Equal("DockerMachine"))
...
func TestSelectMachineForScaleDown(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			machinesRequireUpgrade: internal.NewFilterableMachineCollection(m5),
			expectErr:              false,
		...
		},
		{
			...
			machinesRequireUpgrade: internal.NewFilterableMachineCollection(),
			expectErr:              false,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			selectedMachine, err := selectMachineForScaleDown(tc.cp, tc.machinesRequireUpgrade)
