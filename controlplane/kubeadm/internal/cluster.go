...
type ManagementCluster interface {
	...
	TargetClusterEtcdIsHealthy(ctx context.Context, KCPMachines []*clusterv1.Machine, clusterKey client.ObjectKey) error
	TargetClusterControlPlaneIsHealthy(ctx context.Context, KCPMachines []*clusterv1.Machine, clusterKey client.ObjectKey) error
...
func (m *Management) healthCheck(ctx context.Context, controlPlane *controlplanev1.KubeadmControlPlane, check healthCheck, clusterKey client.ObjectKey) error {
	...
	nodeChecks, err := check(ctx, kcpMachines)
	if err != nil {
...
func (m *Management) TargetClusterControlPlaneIsHealthy(ctx context.Context, kcpMachines []*clusterv1.Machine, clusterKey client.ObjectKey) error {
	// TODO: add checks for expected taints/labels
	cluster, err := m.GetWorkloadCluster(ctx, clusterKey)
	if err != nil {
		return err
	}
	return m.healthCheck(ctx, kcpMachines, cluster.ControlPlaneIsHealthy, clusterKey)
}
...
func (m *Management) TargetClusterEtcdIsHealthy(ctx context.Context, kcpMachines []*clusterv1.Machine, clusterKey client.ObjectKey) error {
	cluster, err := m.GetWorkloadCluster(ctx, clusterKey)
	if err != nil {
		return err
	}
	return m.healthCheck(ctx, kcpMachines, cluster.EtcdIsHealthy, clusterKey)
}
