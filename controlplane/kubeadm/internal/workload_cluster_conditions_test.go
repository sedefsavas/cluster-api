...
func TestUpdateStaticPodConditions(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedKCPCondition: conditions.FalseCondition(controlplanev1.ControlPlaneComponentsHealthyCondition, controlplanev1.ControlPlaneComponentsUnhealthyReason, clusterv1.ConditionSeverityError, "Following machines are reporting control plane components errors: %s", "m1"),
			expectedMachineConditions: map[string]clusterv1.Conditions{
				"m1": {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			w.UpdateStaticPodConditions(ctx, kcp, tt.machines)

			if tt.expectedKCPCondition != nil {
				g.Expect(*conditions.Get(kcp, controlplanev1.ControlPlaneComponentsHealthyCondition)).To(conditions.MatchCondition(*tt.expectedKCPCondition))
			}
...
func TestUpdateStaticPodCondition(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "pending pod with ContainerStatus Waiting should report PodCondition=False, PodProvisioning",
			injectClient: &fakeClient{
				get: map[string]interface{}{
					podkey: fakePod(podName,
						withPhase(corev1.PodPending),
		...
		{
			name: "pending pod with ContainerStatus Terminated should report PodCondition=False, PodFailed",
			injectClient: &fakeClient{
				get: map[string]interface{}{
					podkey: fakePod(podName,
						withPhase(corev1.PodPending),
		...
		{
			name: "pending pod should fallback to report PodCondition=False, PodProvisioning",
			injectClient: &fakeClient{
				get: map[string]interface{}{
					podkey: fakePod(podName,
						withPhase(corev1.PodPending),
		...
		{
			name: "running pod without podReady should report PodCondition=False, PodProvisioning",
			injectClient: &fakeClient{
				get: map[string]interface{}{
					podkey: fakePod(podName,
						withPhase(corev1.PodRunning),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			w.updateStaticPodCondition(machine, node, component, condition)
...
func fakeNode(name string, options ...fakeNodeOption) *corev1.Node { //nolint
	p := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
...
func fakeMachine(name string, options ...fakeMachineOption) *clusterv1.Machine { //nolint
	p := &clusterv1.Machine{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
...
func withNodeRef(ref string) fakeMachineOption { //nolint
